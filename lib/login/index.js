 ...
 * @param {String} userData.user - Name of the user for local reference
...
module.exports = (userData, callback) => {
    ...
    waterfall([
        ...
        (fileData, next) => {
            ...
            let previousData = fileData._auth && _.filter(fileData._auth, [USER, userData.user]);
            ...
            rl.question(USER_OVERRIDE_WARNING, (answer) => {
                ...
                fileData._auth = _.reject(fileData._auth, [USER, userData.user]);
        ...
        (fileData, next) => {
            ...
            !fileData._auth && (fileData._auth = []);
            userData.passkey && (userData.encrypted = true);
            fileData._auth.push(_.pick(userData, [USER, POSTMAN_API_KEY, ENCRYPTED]));
