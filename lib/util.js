...
util = {
    ...
    /**
     * Indicates if the given location is remote
     *
     * @param {String} location - The location of the resource. Can be a file path, URL, Postan-UID or Postman-ID
     */
    isRemote: function (location) {
        return _.isString(location) && !fs.existsSync(location) &&
            ((/^https?:\/\/.*/).test(location) || ID_REGEX.test(location));
    },
     ...
     * @param {Object} options.postmanApiKey - API Key used to load the resources via the Postman API.
    ...
    fetchJson: function (type, location, options, callback) {
        ...
        postmanApiKey && (headers[API_KEY_HEADER] = postmanApiKey);

        // build API URL if `location` is a valid UID/ID and there is no file with the same name.
        if (!fs.existsSync(location) && POSTMAN_API_PATH_MAP[type] && ID_REGEX.test(location)) {
