var _ = require('lodash'),
     ...
     * @param {String|Object} location - The location to load from (file path or URL).
    ...
    externalLoader = function (type, location, options, cb) {
        if (!_.isString(location)) {
            return cb(null, extractModel(location, type));
        }

        // if the location represents a Postman resource,
        // get the data from corresponding instance of PostmanResourceAPI class
        if (!fs.existsSync(location) && options.postmanAPI[type] &&
            (util.POSTMAN_API_URL_REGEX.test(location) || util.POSTMAN_ID_REGEX.test(location))) {
            return options.postmanAPI[type].get(location, cb);
        }

        return util.fetchJson(location, type, function (err, data) {
            if (err) {
                return cb(err);
            }
 ...
 * @param {Object} loadOptions - Set of options related to the load
 * @param {Object} loadOptions.postmanAPI - Set of instances of PostmanResourceAPI class
 * @param {PostmanResourceAPI} loadOptions.postmanAPI.collection - Instance of PostmanResourceAPI for loading
 * Postman-Collections
 * @param {PostmanResourceAPI} loadOptions.postmanAPI.environment - Instance of PostmanResourceAPI for loading
 * Postman-Environments
 * @param {Function} callback - The function called to mark the completion of the configuration load routine.
 * @returns {*}
 */
module.exports = function (options, loadOptions, callback) {
    ...
    async.mapValuesSeries(options, (value, name, cb) => {
        if (value && _.isFunction(configLoaders[name])) {
            return configLoaders[name](value, loadOptions, cb);
        }
