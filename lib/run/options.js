var _ = require('lodash'),
     ...
     * @returns {*}
     */
    apikeyLoader = function ({ apiKeyAlias, _login }, cb) {
        let aliasDetails,
            mutableStdout,
            rl;

        _login && _login._profiles && ([aliasDetails] = _.filter(_login._profiles, ['alias', apiKeyAlias]));
    ...
    externalLoader = function (type, location, options, cb) {
        async.waterfall([
            (next) => {
                ...
                return !options.postmanApiKey && util.isRemote(location) && !(/.\?apikey=./).test(location) ?
                    apikeyLoader(options, next) :
                    next(null, null);
            },
            (apikey, next) => {
...
module.exports = function (options, callback) {
    ...
    async.waterfall([
        (next) => {
            options._cli && config.get(options, { command: 'run' }, next);
        },
        (next) => {
