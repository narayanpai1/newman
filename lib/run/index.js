var _ = require('lodash'),
    ...
    PostmanResourceAPI = require('../api'),
...
module.exports = function (options, callback) {
    ...
    var emitter = new EventEmitter(), // @todo: create a new inherited constructor
        ...
        sessionDetails = { postmanApiKey: options.postmanApiKey,
            postmanApiKeyAlias: options.postmanApiKeyAlias,
            _login: options._login },

        // note that the same object of sessionDetails is passed for both the instances
        // this makes sure if the Session-API-Key is loaded from the rc-file data in one instance,
        // it is reflected across both
        postmanAPI = {
            collection: new PostmanResourceAPI('collection', sessionDetails,
                _.get(options, 'collection.id', options.collection)),

            environment: new PostmanResourceAPI('environment', sessionDetails,
                _.get(options, 'environment.id', options.environment))
        },
    ...
    getOptions(options, { postmanAPI }, function (err, options) {
        if (err) {
            return callback(err);
        }
